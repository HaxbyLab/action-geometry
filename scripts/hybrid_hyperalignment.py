""" 
hybrid_hyperalignment.py
hybrid hyperalignment on surface.
ref: `Busch, Slipski, et al., NeuroImage (2021)` for details.
February 2021
...
from mvpa2.algorithms.hyperalignment import Hyperalignment ##debug
from mvpa2.algorithms.hyperalignment import ProcrusteanMapper ##debug
...
class HybridHyperalignment():
    ...
    def _seed_means(self, measure, queryengine, ds, seed_indices):
        ...
        seed_data = Searchlight(measure, queryengine=queryengine, 
                                nproc=self.nproc, roi_ids=np.concatenate(seed_indices).copy()) #concat might break, error
        if isinstance(ds,np.ndarray):
            ds = Dataset(ds) 
        seed_data = seed_data(ds) #error
    ...
    def _get_connectomes(self, datasets):
        ...
        conn_means = [self._seed_means(MeanFeatureMeasure(), qe, ds, roi_ids) for ds in datasets] #error
    ...
    def _prep_h2a_data(self, response_data, node_indices):
        ...
        for d in h2a_input_data:
            ...
            #zscore(d, chunks_attr=None) #original 
            zscore(np.nan_to_num(d), chunks_attr=None)
    ...
    def __call__(self, datasets):
        ...
        ha_iter1 = SearchlightHyperalignment(queryengine=qe, 
                                      ...
                                      dtype=dtype, tmp_prefix='/backup/data/social_actions/tmp/tmphasl') 
        ...
        ha_iter2 = SearchlightHyperalignment(
            queryengine=qe, 
            nproc=nproc, 
            nblocks=nblocks, 
            mask_node_ids=mask_node_indices,
            dtype=dtype,
            hyperalignment=Hyperalignment(alignment=ProcrusteanMapper(svd='dgesvd')), tmp_prefix='/backup/data/social_actions/tmp/tmphasl')
