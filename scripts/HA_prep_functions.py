...
basedir = '/backup/data/social_actions/scripts/hha'
...
def get_freesurfer_surfaces(hemi):
    ...
    if hemi == 'b':
        ...
        return lh.merge(rh)
    coords1, faces1 = nib.freesurfer.read_geometry(os.path.join(utils.fs_dir,'{lr}h.white'.format(lr=hemi)))
    coords2, faces2 = nib.freesurfer.read_geometry(os.path.join(utils.fs_dir,'{lr}h.pial'.format(lr=hemi)))
    
    np.testing.assert_array_equal(faces1, faces2)
    surf = Surface((coords1 + coords2) * 0.5, faces1)
    return surf
### white and pial directory
### /backup/home/nastase/fmriprep/freesurfer/fsaverage6/surf
...
def get_searchlights(hemi,radius):
    ...
    ds = get_train_data(hemi, 1, num_subjects=1)[0] #change
